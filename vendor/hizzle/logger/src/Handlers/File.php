...
class File extends Handler {
	...
	public function __construct( $log_size_limit = null ) {
		...
		$this->log_size_limit = apply_filters( 'hizzle_log_file_size_limit', $log_size_limit );

		$upload_dir    = wp_upload_dir( null, false );
		self::$log_dir = $upload_dir['basedir'] . '/hizzle-logs';
	...
	protected function add( $timestamp, $level, $message, $source, $context ) {
		...
		if ( $this->should_rotate( $source ) ) {
			$this->log_rotate( $source );
		}

		if ( $this->open( $source ) && is_resource( $this->handles[ $source ] ) ) {
			$result = fwrite( $this->handles[ $source ], $entry . PHP_EOL ); // @codingStandardsIgnoreLine.
		} else {
			$this->cache_log( $timestamp, $level, $message, $source, $context );
		}
	...
	public function clear( $handle ) {
		...
		do_action( 'hizzle_log_clear', $handle );
	...
	public function remove( $handle ) {
		...
		if ( isset( $logs[ $handle ] ) && $logs[ $handle ] ) {
			$file = realpath( trailingslashit( self::$log_dir ) . $logs[ $handle ] );
			if ( 0 === stripos( $file, realpath( trailingslashit( self::$log_dir ) ) ) && is_file( $file ) && is_writable( $file ) ) { // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_is_writable
				$this->close( $file ); // Close first to be certain no processes keep it alive after it is unlinked.
				$removed = unlink( $file ); // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_unlink
			}
			do_action( 'hizzle_log_remove', $handle, $removed );
		}
		return $removed;
	}
	...
	public static function get_log_file_path( $handle ) {
		if ( function_exists( 'wp_hash' ) ) {
			return trailingslashit( self::$log_dir ) . self::get_log_file_name( $handle );
		} else {
			_doing_it_wrong( __METHOD__, esc_html__( 'This method should not be called before plugins_loaded.', 'hizzle-logger' ), '1.0.0' );
			return false;
		}
	}
	 ...
	 * @param array  $context Context will be json encode and stored in database.
	 */
	protected function cache_log( $timestamp, $level, $message, $source, $context ) {
	...
	public function write_cached_logs() {
		foreach ( $this->cached_logs as $log ) {
			$this->add( $log['timestamp'], $log['level'], $log['message'], $log['source'], $log['context'] );
		}
	}
	...
	public static function delete_logs_before_timestamp( $timestamp = 0 ) {
		...
		foreach ( $log_files as $log_file ) {
			$last_modified = filemtime( trailingslashit( self::$log_dir ) . $log_file );

			if ( $last_modified < $timestamp ) {
				@unlink( trailingslashit( self::$log_dir ) . $log_file ); // @codingStandardsIgnoreLine.
			}
		}
	}
	...
	public static function get_log_files() {
		$files  = @scandir( self::$log_dir ); // @codingStandardsIgnoreLine.
		$result = array();
	...
	/**
	 * Returns the default log source.
	 *
	 * @return string
	 */
